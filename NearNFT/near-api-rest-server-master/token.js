...
module.exports = {
     ...
     * @return {string}
     */
    MintNFT: async function (tokenId, receiver_id, title,description,media,copies, contractAccountId, account_id, private_key) {
        ...
        try {
            const tx = await account.functionCall(
                ...
                "nft_mint",
                {
                    ...
                    "token_metadata": { "title": title, "description": description, "media": media, "copies": copies}
                },
                '100000000000000',//Attached Gas Value(Units of gas attached to the transaction)
                '10000000000000000000000');//Attached Deposit Value(Sum of all NEAR tokens transferred from the Signing account to the Receiver account. This includes tokens sent in a Transfer action(s), and as deposits on Function Call action(s).)
